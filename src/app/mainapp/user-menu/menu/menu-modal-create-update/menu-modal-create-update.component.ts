...
export class MenuModalCreateUpdateComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.selectedMenu.subscribe({
      next: res => {
        ...
        this.menuForm.controls['menuHaveChild'].setValue(res.menuHaveChild);
        this.menuForm.controls['menuIsActive'].setValue(res.menuIsActive);
  ...
  disableUrl(eventTarget: any) {
    let boolVal = this.menuForm.controls['menuHaveChild'].value;
    console.log(boolVal);
  ...
  submitForm(value: CreateMenuDto) {
    ...
    if (this.textCreateUpdate == constCreateMenu) {
      ...
      this.MenuSvc.updateMenu(this.menu.id, value).subscribe({
        next: res => this.api.successToastr(res.message, "Success Update"),
        error: this.api.errorHandler
      })
    }
  }
