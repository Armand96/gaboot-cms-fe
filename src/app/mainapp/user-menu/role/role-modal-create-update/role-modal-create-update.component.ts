        ...
        next: res => {
          ...
          for (let index = 0; index < this.menus.length; index++) {
            ...
            if (element.menuHaveChild) {
              ...
              for (let idxJ = 0; idxJ < element.submenus.length; idxJ++) {
        ...
        next: res => {
          ...
          if (Array.isArray(res.menus)) {
            res.menus.forEach(mn => {
              let index = (this.roleForm.controls['roleMenus'].value.findIndex((item: any) => item.menuId == mn.menuId));
              this.menuz().at(index).patchValue({ isChecked: true });
              ...
              for (let idx = 0; idx < mn.submenus.length; idx++) {
                ...
                let subIndex = this.menuz().at(index).value.roleSubmenus.findIndex((subItem: any) => subItem.submenuId == sm.submenuId);
                // console.log(mn, sm)
                this.submenusForm(index).at(subIndex).patchValue({
                  isChecked: true,
